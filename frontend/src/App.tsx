...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F6F8F5] text-[#1D1E18] flex flex-col font-sans selection:bg-[#AAD2BA] selection:text-[#1D1E18]">
      ...
      <header className="border-b border-[#DDE5DE] bg-white/95 backdrop-blur-md sticky top-0 z-40 shadow-sm">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-[68px] flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-[10px] bg-[#6B8F71] flex items-center justify-center text-white font-heading font-extrabold text-base shadow-sm">
              G
            </div>
            <div>
              <div className="font-heading font-bold text-base sm:text-lg text-[#1D1E18] tracking-tight leading-tight">
                Google Business Profile
              </div>
              <div className="text-[11px] text-[#68736B] font-medium hidden sm:block">
          ...
          {/* Navigation Tabs */}
          <nav className="flex items-center bg-[#F6F8F5] p-1 rounded-[10px] border border-[#DDE5DE]">
            <button
              onClick={() => setActiveTab('google')}
              className={`flex items-center gap-2 px-3.5 sm:px-4 py-1.5 rounded-[8px] text-xs font-heading font-semibold transition-all ${
                activeTab === 'google'
      ...
      {/* Main Workspace (max-width: 1440px, padding 24-40px) */}
      ...
      {/* Footer */}
      <footer className="border-t border-[#DDE5DE] bg-white py-5 text-center text-xs text-[#68736B]">
