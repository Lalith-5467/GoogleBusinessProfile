...
export function AdminBusinessesView() {
  ...
  const fetchBusinesses = async () => {
    ...
    setError(null);
    try {
      const data = await adminApi.listUnifiedBusinesses({
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      if (selectedBusiness.source_type === 'GOOGLE_LOCATION') {
        await adminApi.deleteGoogleLocation(selectedBusiness.id);
      } else {
        await adminApi.deleteScrapedBusiness(selectedBusiness.id);
      }
